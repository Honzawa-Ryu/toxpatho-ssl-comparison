"""paper 版の学習曲線・collapse監視・スライド着色 t-SNE
...
def _resolve_input_dir(project_root: Path, params: dict, variant_key: str) -> Path:
    ...
    if not input_dir.is_dir():
        ...
        print("       先に上流フェーズ（0003_20260729_paper_extract_embeddings）を回してください。", file=sys.stderr)
        sys.exit(1)
...
def main() -> None:
    ...
    from lib.analysis.visualize import run
    ...
    run(input_dir=str(run_dir), n=params.get("n_patches", 2000), methods_config=str(project_root / params["methods_config"]))
