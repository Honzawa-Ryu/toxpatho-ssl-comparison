...
- [ ] **所見ラベルによる epoch 別下流評価**（実装済み・未実行, 2026-09-17）
      ...
      ⚠️ **2026-09-23 更新: 対象を広げられるようになった。** 0028 が完走したので
      `model_ep5`〜`model_ep480` が5刻みで96本ある。当初想定の ep20/45/65/85 だけでなく
      **ep5〜480 の全軌跡**に掛けられ、「480 epoch の予算が下流性能を買っているか」に
      決着がつく。どの epoch を抜くかは抽出コスト（1本あたり約30分・11GB）と相談。
      ...
      - スコアは対照群からの Mahalanobis 距離で学習を伴わないため、下記 ABMIL の
        val リーク問題を踏まない
- [x] **exp 0028: peak lr 半減** → **完走 2026-09-23（480/480）。lr 仮説は支持された**
      job 3382307（ep419 で walltime）→ 3398346（`.venv` 破壊で即死・無効）→ 3429521（ep480 完走）。
      最終 train_loss 1.0225 / eff_rank 557.8（0026 は ep89 崩壊・最良 318）。
      投入前に固定した基準「ep100 前後で grad_norm 1.0〜1.5 圏内」は実測 0.8477 で充足。
      変更は `--lr` の1点のみなので **0026 の崩壊原因が lr にあった**と切り分けられる。
      成果物: `model_ssl.pt` と `model_ep5`〜`model_ep480` の **96本**（82GB）。
      詳細は PROJECT_STATUS.md「🎯 exp 0028」。以下は投入時の記録:
...
- [ ] **venv 健全性検査を他の実験の投入スクリプトにも入れる**（2026-09-23）
      `experiments/0028_20260917_dino_lr_half/resume.sh` の点検項目7（`pyvenv.cfg` の
      `home` / `include-system-site-packages` と `timm` の有無）を、他の `run_slurm.sh` /
