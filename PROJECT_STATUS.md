...
最終更新: 2026-09-02（セッション9: **clip_grad 0.3 で崩壊解消**。0026 を ep20 から再開中(job 3280970)。下記「🦕 DINO崩壊調査」）
...
1. **`0026`（job 3280970）が ep20 から継続中**。480 epoch 完走まで `--resume` で再投入する。
   ```
   qsub experiments/0026_20260901_dino_clipgrad03/run_slurm.sh
   ```
   ⚠️ abort/walltime切れで `model_ssl.pt` が書かれていると `--resume` が
   「training complete, nothing to resume」で即終了する。継続したいときは
   `model_ssl.pt` を退避してから再投入すること。
2. 監視項目（毎epochログ）:
   - `ln_gain` が 1.0 付近から緩やかに下がるのは正常（勾配由来）。**急落したら wd の再発**
   - `train_loss` が **11.0904**（= ln(65536)）に張り付かないこと
   - `eff_rank` が上昇傾向であること
3. 0017/0021/0023/0024/0025 の重みは事前学習済みモデルとして使用不可。
4. MAE / SimSiam の実験（0013/0016/0018/0014）は未実行のまま。修正後の設定で投入すること。
5. `freeze_last_layer` を CLI に出す件は保留（0026 が順調なので当面不要）。
