...
### 観察ログ（2026-08-29 19:30時点、epoch 9 まで。ジョブ進行中）

**スケジュール配線は正しく動作していることを確認済み**（0024の teacher_temp が
ep5で0.0450 = 理論値 `0.04 + 0.03 × (4×781+780)/(30×781)` と完全一致、
0023の momentum は 0.996 から微増、それぞれ他方は固定）。0021で疑われた
「有効にしたつもりで固定のまま」ではない。
...
| 1 | 8.4604 / gn **1.99** | — |
| 2 | 4.5532 / gn **5.42** ⚠️ | — |
| 3 | 8.9401 / gn 0.18 | 8.9246 / gn 0.13 |
| 4 | **9.0109** 崩壊 / gn 0.005 | 8.9059 / gn 0.13 |
| 5 | 9.0108 / gn 0.0006 | 8.8617 / gn 0.14 |
| 6 | 9.0100 / gn 0.006 | 8.7819 / gn 0.15 |
| 7 | 8.9576 回復開始 / gn 0.09 | 8.7700 / gn 0.15 |
| 8 | 8.4064 / gn 0.29 | 8.9040 / gn 0.09 |
| 9 | **7.9934** 回復継続 / gn 0.34 | **9.0108** 崩壊 / gn 0.002 |

**⚠️ 切り分けは綺麗に分離しなかった。両軸とも崩壊する。**
当初「0024は健全＝温度は無実」と読んだが epoch 9 で崩壊したため撤回。

崩壊タイミングを全ランで並べると、**すべて warmup 中〜直後の lr 上昇局面**で起きている:

| ラン | 崩壊epoch | その時のlr | teacher設定 | その後 |
|---|---|---|---|---|
| 0023 | **4** | 6.07e-4 | momentum 0.996（速い） | ep7から回復中 |
| 0024 | **9** | 1.60e-3 | 温度warmup | 観察中 |
| 0017 | 12 | 2.00e-3 | 両方固定 | ep34頃回復し100ep完走 |
| 0021 | 15 | 1.99e-3 | 両方 + step粒度 | 恒久崩壊 |

teacher設定は「**どれだけ早く落ちるか**」を変えているだけで、
一様崩壊(loss = ln(8192) = 9.0109)への転落自体は全ランに共通する現象。
teacherが速いほど早く落ちる、という順序になっている。

**→ 主因の最有力候補は、両軸に共通する「勾配クリッピング欠如 + warmupでのlr上昇」。**
0023 は崩壊直前の **epoch 2 で grad_norm 5.4195 のスパイク**を記録している
（loss 4.55 まで突っ込んだ直後に 8.94 → 9.01 へ吹き飛んだ）。
公式の `clip_grad=3.0` ならこの 5.42 はクリップされていた。
0017（momentum 0.9995）は grad_norm が最大 0.86 に収まりスパイクしていない。
**「momentum 0.996 が崩壊させる」のではなく「momentum 0.996 は勾配を大きくする →
クリッピングが無いのでスパイクで吹き飛ぶ」という因果の可能性が高い。**

**副産物: `CollapseMonitor` の判定指標(eff_rank)は鈍すぎる。**
0023 の ep5 は loss が ln(8192) ちょうど＝出力が完全な一様分布、
feat_std 0.0554（健全時0.58）、alignment 0.0032（健全時0.24）と壊滅的なのに
**eff_rank は 31.99** で、閾値5.0では発火しない。0021でも eff_rank が5を切るのに
epoch 40 近くかかった。**feat_std / alignment の方が桁違いに鋭敏**なので、
3点セット実装時に判定指標を見直すこと。
