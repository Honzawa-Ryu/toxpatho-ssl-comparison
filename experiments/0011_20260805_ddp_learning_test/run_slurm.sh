#!/bin/bash
#SBATCH --job-name=0011_20260805_ddp_learning_test
#SBATCH --partition=x-large-andre01
#SBATCH --output=/workspace/andre01/honzawa/01-toxpatho/toxpatho-ssl-comparison/logs/0011_20260805_ddp_learning_test/%j_0011_20260805_ddp_learning_test.out
#SBATCH --error=/workspace/andre01/honzawa/01-toxpatho/toxpatho-ssl-comparison/logs/0011_20260805_ddp_learning_test/%j_0011_20260805_ddp_learning_test.out
...
# #SBATCH --output=/workspace/andre01/honzawa/01-toxpatho/toxpatho-ssl-comparison/logs/0011_20260805_ddp_learning_test/%A_%a_0011_20260805_ddp_learning_test.out
# #SBATCH --error=/workspace/andre01/honzawa/01-toxpatho/toxpatho-ssl-comparison/logs/0011_20260805_ddp_learning_test/%A_%a_0011_20260805_ddp_learning_test.out
# #SBATCH --array=0-N
#
...
export EXP_NAME="0011_20260805_ddp_learning_test"
...
# Single run（デフォルト）
#
# 1ノード内マルチGPU（DDP）のテストなので、python 単体ではなく torchrun 経由で
# 起動する（Goal.yaml のマルチGPU移行チェックリスト項目4）。--nproc_per_node は
# 上の #SBATCH --gres=gpu:N と必ず一致させること（ずれると余ったプロセスが
# GPUを取り合うか、確保したGPUが遊ぶ）。--standalone は単一ノード内の
# rendezvous を torchrun 側に自動選択させる（MASTER_ADDR/PORT の手動設定不要）。
# =====================================================

NPROC_PER_NODE=2
RUN_MODE="single"
RUN_COMMAND="torchrun --standalone --nproc_per_node=${NPROC_PER_NODE} ${PYTHON_PATH} --config config.yml"
