...
#
# このファイルはテンプレート/capsuleリポジトリ本体に同梱されている。
# `.bashrc.d/*.sh` は `.bashrc` から無条件で source されるため、この
# リポジトリを `~/.bashrc` から読み込んでいれば gstart/gpush は自動的に
# 有効になる（「テンプレート外の個人設定に置く」運用は取っていない）。

# ==========================================
# Git 初回セットアップ（最小限）
# ==========================================

# ⚠️ このgit hookはリポジトリローカルではなく `git config --global core.hooksPath`
#    でグローバルに設定される。つまりユーザーが手元で扱う**全てのリポジトリ**の
#    hooksパスがこのディレクトリに向けられ、各リポジトリ固有の `.git/hooks/` は
#    無効化される。他プロジェクトで独自のgit hookを使っている場合は上書きされる
#    ため注意すること（解除する場合は `git config --global --unset core.hooksPath`）。
_setup_git_config() {
